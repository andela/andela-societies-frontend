...
// helpers
import { hasAllowedRole, dateFormatter, filterActivitiesByStatus, statsGenerator } from '../helpers';
...
import { fetchAllActivities, verifyActivity, verifyActivitiesOps } from '../actions/allActivitiesActions';
...
class VerifyActivities extends Component {
	...
	static getDerivedStateFromProps(props, state) {
	  if (props.userRoles.length) {
	    ...
	    if (hasAllowedRole(userRoles, [SUCCESS_OPS])) {
	      showTabs = true;
	      filteredActivities = filterActivitiesByStatus(allActivities, PENDING).filter(activity => activity.society.name.toLowerCase() === selectedSociety);
	    } else {
	      filteredActivities = filterActivitiesByStatus(allActivities, IN_REVIEW).filter(activity => activity.society.name.toLowerCase() === societyName.toLowerCase());
	    }
	...
	handleSelectAllClick = () => {
	  ...
	  const selectedActivities = filteredActivities
	    .filter(activity => !isSelectAllChecked && activity.id)
	...
	handleChangeTab = (event, title) => {
	  ...
	  const selectedSocietyActivities = filterActivitiesByStatus(this.props.allActivities, PENDING).filter(activity => activity.society.name.toLowerCase() === title.toLowerCase());
	  this.setState({
  ...
  renderLayout() {
	...
	render() {
	  ...
	  const {
	    message, filteredActivities, showTabs, selectedStatus, selectedSociety, selectedActivity,
	  } = this.state;
	  let snackBarMessage = '';
	  if (message) {
	  ...
	  return (
  <Page selectedItem={selectedActivity} deselectItem={this.deselectActivity}>
  ...
  disabled={disableButton}
	              />
  <div className='activities'>{this.renderLayout()}</div>
</div>
	          )}
  </div>
    </div>
  <aside className='sideContent'>
  <Stats stats={statsGenerator(filteredActivities, 'Verify activities', 'Total points')} />
	      </aside>
	      {snackBarMessage}
	    </Page>
	  );
